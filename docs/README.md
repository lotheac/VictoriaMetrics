...
```shellhelp
  ...
  -maxLabelValueLen int
     The maximum length of label values in the accepted time series. Series, with longer label values are ignored. Validation is applied after relabeling defined at -relabelConfig. In this case the vm_rows_ignored_total{reason="too_long_label_value"} metric at /metrics page is incremented (default 4096)
  -maxLabelsPerTimeseries int
     The maximum number of labels accepted per time series. Series with superfluous labels are ignored. Validation is applied after relabeling defined at -relabelConfig. In this case the vm_rows_ignored_total{reason="too_many_labels"} metric at /metrics page is incremented (default 30)
